...
const (
	off int64 = 512 * 64
	dtb       = "rockchip/rk3588-nanopc-t6-lts.dtb"
)

func main() {
	adapter.Execute(&nanopcT6LTSInstaller{})
}

type nanopcT6LTSInstaller struct{}

type nanopcT6LTSExtraOptions struct {
	SPIBoot bool `yaml:"spi_boot,omitempty"`
}

func (i *nanopcT6LTSInstaller) GetOptions(extra nanopcT6LTSExtraOptions) (overlay.Options, error) {
	kernelArgs := []string{
		"console=tty1",
...
func (i *nanopcT6LTSInstaller) Install(options overlay.InstallOptions[nanopcT6LTSExtraOptions]) error {
	if !options.ExtraOptions.SPIBoot {
		uBootBin := filepath.Join(options.ArtifactsPath, "arm64/u-boot/nanopc-t6-lts/u-boot-rockchip.bin")
